...
__export(main_exports, {
  ...
  makeHeadingsFoldable: () => makeHeadingsFoldable
});
...
function initVim(CodeMirror2) {
  ...
  var exCommands = {
    ...
    substitute: function(cm, params) {
      ...
      doReplace(cm, confirm, global2, lineStart, lineEnd, cursor, query, replacePart, params.callback);
    },
    /** @arg {CodeMirrorV} cm @arg {ExParams} params*/
...
// codemirror-lang-orgmode/dist/index.js
...
function checkTodoKeyword(s, input) {
...
function checkPreviousWord(input, anti_peek_distance = -1) {
...
var title_tokenizer = new import_lr.ExternalTokenizer((input, stack) => {
  ...
  while (!isEndOfLine(c)) {
    while (c != COLON && !isEndOfLine(c)) {
      ...
      if (!todo_keyword_already_matched && checkTodoKeyword(s)) {
        return;
      }
    }
    if (isEndOfLine(c)) {
...
var todokeyword_tokenizer = new import_lr.ExternalTokenizer((input, stack) => {
  const words = [...global.todoKeywords, ...global.doneKeywords];
...
var endofline_tokenizer = new import_lr.ExternalTokenizer((input, stack) => {
...
var section_tokenizer = new import_lr.ExternalTokenizer((input, stack) => {
    ...
    while (!isEndOfLine(c)) {
      ...
      if (first_line) {
        if (c === COLON) {
          ...
          if (!could_be_property_drawer && (planning_word.toUpperCase() === "DEADLINE" || planning_word.toUpperCase() === "SCHEDULED" || planning_word.toUpperCase() === "CLOSED")) {
            return;
          }
        }
...
var propertydrawer_tokenizer = new import_lr.ExternalTokenizer((input, stack) => {
...
var parser = import_lr.LRParser.deserialize({
  version: 14,
  states: "%|QQOcOOO`OpO'#CbOOOa'#Ch'#ChQQOcOOOeO^O'#CdOpOcO'#CcOOOO'#Ci'#CiQZOQOOQZOQOOQ!UObOOOOOa,58|,58|OOOa-E6f-E6fO!^OrO,59OO!fOSO,59OO!kOUO,59OO!sOpO'#CgOOOa'#Cj'#CjO!xOcO,58}OOO`'#Ck'#CkO#^ObO,58}O#^ObO,58}OOOO-E6g-E6gOOOa1G.j1G.jO#iOpO1G.jO#nOrO1G.jO#vOSO1G.jOOOa,59R,59ROOOa-E6h-E6hO#{ObO1G.iO#{ObO1G.iOOO`-E6i-E6iOOOa7+$U7+$UO$WOpO7+$UO$]OrO7+$UO$eObO7+$TOOOa<<Gp<<GpO$pOpO<<GpOOOaAN=[AN=[",
  stateData: "$u~OPXOSWObPOcSO~OaYO~OQ[OR^OX]O~OPdOSbObPOd_O`VXcVX~OSWOcSO~OYgOafO~OQhO~OQhOXiO~OajO~OPmOSbObPOd_O`VacVa~OSbO`VacVa~OaoO~OYpOaoO~OQqO~OSbO`VicVi~OasO~OYtOasO~OSbO`VqcVq~OauO~O",
  goto: "!m`PPPPPPaipPPew}!W!^SQORT`TaZUORVWXZTORVWXQRORZRWVORWXReVQaTRkaQcTSladUnclrRrm",
  nodeNames: "\u26A0 PropertyDrawer Title TodoKeyword Section Program Comment Block Heading Priority Tags Planning",
  maxTerm: 20,
  skippedNodes: [0],
  repeatNodeCount: 4,
  tokenData: "'i~RXXYnpqnst!oz{!t![!]#]!e!f$|!f!g%w!u!v&s!}#Oz~qRXYnpqn!}#Oz~}Pst!Q~!TR!Q![!^!c!}!^#T#o!^~!aP#P#Q!d~!iQX~XY!dpq!d~!tOb~~!wRXY#Qpq#Qz{!t~#VQc~XY#Qpq#Q~#`Vst#uuv#u!Q![#u!b!c#u!c!}#u#R#S#u#T#o#u~#xWst#uuv#u!Q![#u![!]$b!b!c#u!c!}#u#R#S#u#T#o#u~$gVY~st#uuv#u!Q![#u!b!c#u!c!}#u#R#S#u#T#o#u~%PP!n!o%S~%VP!q!r%Y~%]P!u!v%`~%cP!g!h%f~%iP!f!g%l~%oP![!]%r~%wOd~~%zP!g!h%}~&QP!c!d&T~&WP!f!g&Z~&^P!n!o&a~&dP!k!l&g~&jP!p!q&m~&pP!g!h%l~&vP!e!f&y~&|P!j!k'P~'SP!g!h'V~'YP!f!g']~'`P!w!x'c~'fP!n!o%`",
  tokenizers: [propertydrawer_tokenizer, 0, title_tokenizer, todokeyword_tokenizer, section_tokenizer, endofline_tokenizer],
  topRules: { "Program": [0, 5] },
  tokenPrec: 0
});
var OrgmodeLanguage = import_language2.LRLanguage.define({
  parser: parser.configure({
    props: [
      (0, import_highlight.styleTags)({
        "Block": import_highlight.tags.atom,
...
function Orgmode() {
  return new import_language2.LanguageSupport(OrgmodeLanguage);
}
...
var makeHeadingsFoldable = import_language3.foldService.of((state, from, to) => {
  let is_heading = false;
  let block_to = null;
  let heading_level = null;
  for (let node = (0, import_language3.syntaxTree)(state).resolveInner(to, -1); node; node = node.parent) {
...
var OrgView = class extends import_obsidian.TextFileView {
  constructor(leaf) {
    ...
    this.extensions = [
      ...
      Orgmode(),
      (0, import_language3.syntaxHighlighting)(myHighlightStyle),
    ...
    Vim.defineEx("write", "w", () => {
      var _a2;
      (_a2 = this.app) == null ? void 0 : _a2.commands.executeCommandById("editor:save-file");
    });
  }
  getDisplayText() {
    if (this.file) {
